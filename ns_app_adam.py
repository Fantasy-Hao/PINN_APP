...
class PINN(nn.Module):
    ...
    def f(self, x, y, t):
        ...
        # x-momentum equation: ∂u/∂t + u∂u/∂x + v∂u/∂y = -∂p/∂x + (1/Re)(∂²u/∂x² + ∂²u/∂y²)
        momentum_x = u_t + u * u_x + v * u_y + p_x - (1/self.Re) * (u_xx + u_yy)

        # y-momentum equation: ∂v/∂t + u∂v/∂x + v∂v/∂y = -∂p/∂y + (1/Re)(∂²v/∂x² + ∂²v/∂y²)
...
def loss_fun(model, params, inputs):
    ...
    # Calculate initial condition loss (fluid at rest at t=0)
...
def main():
    ...
    plt.grid(True)
    plt.savefig('./logs/ns_loss.png', dpi=300, bbox_inches='tight')
    plt.show()
