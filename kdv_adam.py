...
def generate_training_data(x_min=-15.0, x_max=15.0, t_min=0.0, t_max=5.0, nx=200, nt=100):
    ...
    return x_lb, t_b, x_ub, t_b, x_0, t_0, u_0, x_f, t_f, x_tensor, t_tensor, Exact, X, T
...
def train(model, n_epochs, x_lb, t_lb, x_ub, t_ub, x_0, t_0, u_0, x_f, t_f):
    ...
    for epoch in range(n_epochs):
        ...
        # Calculate boundary condition loss (periodic boundary conditions)
...
def evaluate_model(model, x, t, Exact, X, T):
    ...
    ax.set_ylabel('$t$')
    ax.set_zlabel('$u(x,t)$')
    ax.set_title('Exact Solution (Soliton)')
    ...
    plt.tight_layout()
    plt.savefig('./logs/kdv_solution.png', dpi=300, bbox_inches='tight')
    plt.show()
