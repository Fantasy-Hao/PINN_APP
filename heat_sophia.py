...
def train(model, optimizer, epochs, n_points):
    ...
    for epoch in range(epochs):
        ...
        if epoch % 1000 == 0:
            print(
                f'Sophia - Epoch {epoch}, Loss: {loss.item():.6e}, PDE Loss: {loss_f.item():.6e}, IC Loss: {loss_initial.item():.6e}, BC Loss: {loss_bc.item():.6e}')
...
def evaluate_model(model, n_points=100):
    ...
    plt.tight_layout()
    plt.savefig('./logs/heat_sophia_results.png', dpi=300)
    plt.show()
    ...
    ax.legend()
    ax.grid(True)
    plt.savefig('./logs/heat_sophia_time_slices.png', dpi=300)
    plt.show()
...
def main():
    ...
    optimizer = SophiaG(model.parameters(), lr=1e-4, betas=(0.965, 0.99), rho=0.1, weight_decay=0)
    ...
    plt.grid(True)
    plt.savefig('./logs/heat_sophia_loss.png', dpi=300)
    plt.show()
    ...
    # Save model
    # torch.save(model.state_dict(), './logs/heat_sophia_model.pt')
    # print("Model saved as './logs/heat_sophia_model.pt'")
