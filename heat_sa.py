"""
Heat Equation PINN Solver with Dual Annealing Optimization.
...
def train_dual_annealing(model, inputs, target, maxiter=1000, initial_temp=5230, restart_temp_ratio=2.e-5):
    ...
    def callback(x, f, context):
        ...
        if len(loss_history) % 10 == 0:  # Print every 10 iterations
            print(f'Dual Annealing -  Epoch {len(loss_history)}, Loss: {f:.6e}')
...
# Training function
def train(model, n_points, maxiter=1000):
    # Domain bounds
    x_min, x_max = 0.0, 1.0
    t_min, t_max = 0.0, 1.0
...
def evaluate_model(model, n_points=100):
    ...
    # ax.legend()
    # ax.grid(True)
    # plt.savefig('./logs/heat_sa_time_slices.png', dpi=300)
    # plt.show()
...
def main():
    ...
    plt.grid(True)
    plt.savefig('./logs/heat_sa_loss.png', dpi=300)
    plt.show()
