...
def train(model, optimizer, epochs=10000, n_points=100):
    ...
    for epoch in range(epochs):
        ...
        if epoch % 1000 == 0:
            print(
                f'Epoch {epoch}, Loss: {loss.item():.6f}, PDE Loss: {loss_f.item():.6f}, BC Loss: {loss_bc.item():.6f}')
...
def main():
    ...
    losses = train(model, optimizer, epochs=10000, n_points=1000)
    ...
    # Save model
    # torch.save(model.state_dict(), 'poisson_model.pt')
    # print("Model saved as 'poisson_model.pt'")
